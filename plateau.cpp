...
bool Plateau::permission_bouge(Piece* p, Case c){ // on teste les permissions de bouger en connaissant le plateau, string pour indiquer quel piece bouge
    ...
    if (p->get_name().compare(std::string("roi"))==0){
        if (p->permission_bouge(c) && get(c)==nullptr) return true;
        else return false;
    }
